  ...
  for(edm::View<pat::Muon>::const_iterator iMuon1 = thePATMuonHandle->begin(); iMuon1 != thePATMuonHandle->end(); ++iMuon1) 
      ...
      for(edm::View<pat::Muon>::const_iterator iMuon2 = iMuon1+1; iMuon2 != thePATMuonHandle->end(); ++iMuon2) 
	{
	  ...
	  //if(iMuon1->track()->pt()<3.0) continue;
          if(iMuon1->track()->pt()<2.0) continue; // Changed By Mobassir 4.0 to 2.0 
 
          //if(iMuon2->track()->pt()<4.0) continue;
          //if(iMuon1->track()->pt()<3.0) continue;
          if(iMuon2->track()->pt()<2.0) continue; // Change By Mobassir 4.0 to 2.0
	  ...
	  //RefCountedKinematicTree psiVertexFitTree = VertexFitter->fit(psiParticles);
          RefCountedKinematicTree psiVertexFitTree;
          psiVertexFitTree->movePointerToTheTop(); 

	  RefCountedKinematicParticle psi_vFit_noMC = psiVertexFitTree->currentParticle();//masa del J/psi 
	  //RefCountedKinematicParticle psi_vFit_noMC = VertexFitter->currentParticle();//masa del J/psi
	  ...
	  for(ix = 0; (unsigned)ix<thePATTrackHandle->size(); ix++){
	    ...
	    if(trackView.pt()<0.5) continue; // Changed by Mobassir 1 to 0.5 
	  ...
	  // //Now let's see if these two tracks make a vertex
		...
		try{
		  ...
		  //DsVertexFitTree = TIter->fit(pionParticles); 
		}
		catch(...) {
